...
export function EquipmentModule({ equipment, jobs, isAdmin, onEquipmentChanged }: Props) {
  ...
  const filteredEquipment = useMemo(() => {
    ...
    return equipment.filter((item) =>
      [
        ...
        item.ownership_type,
        item.current_site,
        item.current_job_id ? jobNameById.get(item.current_job_id) : ""
      ]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
  ...
  function EquipmentListCard({ item }: { item: Equipment }) {
      ...
      <aside className="card h-fit lg:sticky lg:top-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs font-bold text-slate-500">{equipment.length} records</p>
          </div>
        ...
        <div className="mt-4 max-h-[70vh] space-y-3 overflow-y-auto pr-1">
          ...
          {filteredEquipment.length === 0 ? (
            ...
            filteredEquipment.map((item) => <EquipmentListCard key={item.id} item={item} />)
          )}
        </div>
      </aside>
      ...
      <section className="card">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-2xl font-black">
            {selectedId === "new" ? "New Equipment" : form.name || "Equipment Info"}
          </h2>
